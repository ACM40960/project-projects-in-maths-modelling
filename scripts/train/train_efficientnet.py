...
def get_block_channels(block):
    """Get output channels from EfficientNet block"""
    if hasattr(block, 'conv_pwl'):  # Point-wise linear conv 
        return block.conv_pwl.out_channels
    elif hasattr(block, 'conv_pw'):  # Point-wise conv
        return block.conv_pw.out_channels
    else:
        # Fallback - find the last conv layer
        for module in reversed(list(block.modules())):
            if isinstance(module, nn.Conv2d):
                return module.out_channels
        return 1280  # EfficientNet-S default

# --- 1. Model -------------------------------------------------------------
class CBAMEffNet(nn.Module):
    """
    EfficientNetB0 with three CBAM layers inserted after the
    last three block stages of the backbone.
    """
    def __init__(self, n_cls, dropout=0.2):
        super().__init__()
        self.backbone = timm.create_model('tf_efficientnetv2_s', pretrained=True, num_classes=0)

        # We will insert CBAM after the last 3 block stages
        n_stages = len(self.backbone.blocks)
        cbam_stage_indices = [n_stages - 3, n_stages - 2, n_stages - 1]

        self.cbams = nn.ModuleDict()
        for idx in cbam_stage_indices:
            # Get the last block within the stage to find the output channel count
            last_block_in_stage = self.backbone.blocks[idx][-1]
            channels = get_block_channels(last_block_in_stage)
            self.cbams[str(idx)] = CbamModule(channels)
            print(f"CBAM layer added after stage {idx} with {channels} channels.")
    ...
    def forward(self, x):
        ...
        # bn1 is BatchNormAct2d, which applies activation automatically
        x = self.backbone.bn1(x)

        #  Blocks with CBAM insertion
        for i, stage in enumerate(self.backbone.blocks):
            x = stage(x)
            # Apply CBAM if it's one of the targeted stages
            if str(i) in self.cbams:
                x = self.cbams[str(i)](x)

        # Head 
        x = self.backbone.conv_head(x)
        # bn2 is also BatchNormAct2d
...
class TailAwareFolder(torch.utils.data.Dataset):
    ...
    def __init__(self, root, main_tf, tail_tf, tail_ids):
        self.ds        = ImageFolder(root)             # inherits class list
...
train_tf = tv.transforms.Compose([
    ...
    tv.transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.2, hue=0.02),
    ...
    def forward(self, logits, target):
        ...
        if target.ndim == 1:                           # hard labels
            target_onehot = torch.nn.functional.one_hot(
                target, num_classes=logits.size(1)).float()
        else:                                          # soft labels
            target_onehot = target.float()

        # add per-class logit bias  (log wᵢ)
...
if __name__ == '__main__':
    ...
    tail_ids   = torch.where(cls_counts < 200)[0].tolist()   # tune threshold
    ...
    mix_collate = TailMixCollate(num_classes=len(train_ds.classes),
                             tail_ids=tail_ids, alpha=0.2, p=0.5)
    ...
    model = CBAMEffNet(n_cls=len(train_ds.classes)).to(device)

    # Loss function with class weights for extra imbalance handling
    class_weights = 1. / cls_counts.float()
    class_weights = class_weights / class_weights.sum() * len(class_weights)  # normalize
    crit = CBFocalLoss(samples_per_cls=cls_counts, beta=0.999, gamma=1.5).to(device)
    ...
    sched = OneCycleLR(opt, max_lr=[1e-4, 3e-4], epochs=50, steps_per_epoch=len(train_dl))
    scaler = GradScaler()
    ...
    output_dir = Path("efficientnet_v2_s_classaware_run2")
    ...
    for epoch in range(50):
        ...
        pbar = tqdm(train_dl, desc=f"Epoch {epoch+1:02d}/50")
        for x, y in pbar:
        ...
        with torch.no_grad():
            for x, y in val_dl:
                ...
                _, pred_topk = out.topk(3, 1, True, True)   # shape (B, 3)
                correct = pred_topk.eq(y.view(-1, 1))       # (B, 3) bool
